...
export function SkillForm({ open, onClose, skill }: Props) {
  const [tab, setTab] = useState<Tab>(skill ? "write" : "write");
  const [isPending, startTransition] = useTransition();
  const [error, setError] = useState<string | null>(null);
  const [dragOver, setDragOver] = useState(false);
  const fileRef = useRef<HTMLInputElement>(null);

  // Controlled write fields (pre-filled after upload parse)
      ...
      <DialogContent className="admin-dialog max-w-2xl">
        ...
        {tab === "upload" && !skill && (
          <div className="flex flex-col gap-4">
            ...
            <div className="rounded-xl border border-[var(--color-border)] bg-[var(--color-surface-muted)] px-4 py-3 text-xs text-muted-foreground space-y-1">
              <p className="font-medium text-[var(--color-text-secondary)]">File requirements</p>
              <ul className="list-disc list-inside space-y-0.5">
                <li>A <code>.md</code> file may include YAML frontmatter with <code>name</code> and <code>description</code></li>
                <li>A <code>.zip</code> file must contain a <code>SKILL.md</code> file</li>
              </ul>
            </div>
